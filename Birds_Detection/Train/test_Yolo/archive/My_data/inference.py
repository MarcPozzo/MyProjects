...
file="."
#chdir("/home/marcpozzo/Documents/Tutoriels-master/Tensorflow/tutoriel27/")
...
for i in range(len(images)):
  ...
  #prediction=model(np.array([images[i]]).reshape(1,256,192,3))
  ...
  if key==ord('q'):
    quit()
"""
images, labels, labels2=common.read_imagettes_2()
images=np.array(images, dtype=np.float32)/255
labels=np.array(labels, dtype=np.float32)
index=np.random.permutation(len(images))
images=images[index]
...
checkpoint=tf.train.Checkpoint(model=model)
#checkpoint.restore(tf.train.latest_checkpoint("./training/"))
checkpoint.restore(tf.train.latest_checkpoint("/home/marcpozzo/Documents/Tutoriels-master/Tensorflow/training1000/"))
