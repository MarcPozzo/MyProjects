...
def extract_fp_court(path_images,name_test,name_ref,folder,CNNmodel,coverage_threshold=0.5,contrast=-5,blockSize=53,blurFact=15,
    ...
    #On pourrait insérer un ou plusieurs filtres ici
    ...
    (liste_Diff_animals,dict_anotation_index_to_classe,liste_DIFF_birds_defined,liste_DIFF_birds_undefined,birds_defined_match,liste_Diff_birds_this_class,
     ...
     liste_DIFF_pigeon,liste_DIFF_other_animals)=class_imagettes(generate_square,coverage_threshold, nom_classe,imagettes_target,dic_labels_to_num)  
    ...
    #On vérifie que l'intersection entre la catégorie prédite (index) correspond bien à la catégorie réel de l'imagette (diff) 
    ...
    #On garde seulement la probabilité correspondant à la classe prédite (pratique en cas de cut)
    others_estimates = [max(estimates[i]) for i in (index_others)]
    birds_estimates = [max(estimates[i]) for i in (index_birds)]
    ...
    table["max_cat"]=FP_estimates+animals_match_estimates
...
def predict_under_thres(path_images,name_test,name_ref,folder,CNNmodel,coverage_threshold=0.99,contrast=-5,blockSize=53,blurFact=15,
    ...
    #lite_table_quantile=[liste_table[i] for i in index_possible_animals ]   
    ...
    #On vérifie que l'intersection entre la catégorie prédite (index) correspond bien à la catégorie réel de l'imagette (diff) 
    print("liste_Diff_birds",liste_Diff_birds)
    print("index_birds",index_birds)
    birds_pr_birds=set(liste_Diff_birds).intersection(index_birds)
    others_pr_birds=set(liste_DIFF_not_matche).intersection(index_birds)
    FP=set(liste_DIFF_not_matche).intersection(index_animals)
    TP_birds=list(set(liste_DIFF_corbeau).intersection(index_corbeau))+list(set(liste_DIFF_faisan).intersection(index_faisan))+list(set(liste_DIFF_pigeon).intersection(index_pigeon))
    ...
    print("birds_pr_birds",birds_pr_birds)
    print("oiseaux estimés fonction",birds_pr_bird_estimates)
    if thresh_active==True:
        #birds_pr_birds_thresh=len([max(estimates[i])>thresh for i in (birds_pr_birds)])
        """
        birds_pr_birds_max=[max(estimates[i]) for i in (list(birds_pr_birds))]
        FP_birds_max=[max(estimates[i]) for i in (FP_birds_estimates)]
        """
        birds_pr_birds_thresh=[i for i in birds_pr_bird_estimates if i > thresh]
        FP_birds_thresh=[i for i in FP_estimates if i > thresh]
        print("birds_pr_birds_thresh",birds_pr_birds_thresh)
        print("FP_birds_thresh",FP_birds_thresh) 
        
        
       
        ln_birds_pr_birds_thresh=len([i for i in birds_pr_bird_estimates if i > thresh])
        ln_FP_birds_thresh=len([i for i in FP_birds_estimates if i > thresh])      
        
        """
        birds_pr_birds_thresh=len([max(i) for i in max(estimates[i]) if max(i) > thresh])
        birds_pr_birds_thresh=len([max(i) for i in max(estimates[i]) if max(i) > thresh])
        
        birds_pr_birds_max=[max(estimates[i])>thresh for i in (birds_pr_birds)]
        birds_pr_birds_thresh=len([i for i in max(estimates[i]) if i > thresh])
        FP_birds_thresh=len([i for i in max(estimates[i]) if i > thresh])
        FP_birds_thresh=len([max(estimates[i])>thresh for i in (FP_birds_estimates)])
        """
        print("le nombre d'oiseaux trouvés est :",birds_pr_birds_thresh)
        print("le nombre d'oiseau trouvés par erreur est :",FP_birds_thresh)
        return ln_FP_birds_thresh,ln_birds_pr_birds_thresh
    #
    if thresh_active==False:
        return ln_FP_birds_estimates, ln_birds_pr_bird_estimates
...
def draw_fp(path_images,name_test,name_ref,folder,CNNmodel,coverage_threshold=0.5,contrast=-5,blockSize=53,blurFact=15,
    ...
    TP_birds_maxs=[max(estimates[i]) for i in (TP_birds)]
    FP_birds_maxs=[max(estimates[i]) for i in (FP)]
    print("True Positif", TP_birds_maxs)
    print("FP_birds_estimates",FP_birds_maxs)
    
    
    index_TP=[]
    #for i, j in enumerate(TP_birds_estimates):
    for i, j in enumerate(TP_birds_maxs):
        if j >0.9 :
            index_TP.append(i)
    ...
    birds_square=generate_square.loc[TP_birds]
...
#return extract_dist_by_type

def extract_dist_by_type_essai(name_test,name_ref,folder,CNNmodel,numb_classes,mask=True,coverage_threshold=0.99,contrast=-5,blockSize=53,blurFact=15,
    ...
    #On garde seulement la probabilité correspondant à la classe prédite 
    others_estimates = [max(estimates[i]) for i in (index_others)]
    birds_estimates = [max(estimates[i]) for i in (index_birds)]
    other_animals_estimates = [max(estimates[i]) for i in (index_other_animals)]
    FP_birds_estimates=[max(estimates[i]) for i in (others_pr_birds)]
    ...
    batchImages_match_animals=[batchImages_stack_reshape[i] for i in liste_Diff_animals ]  
    
    Diff_image_FP=list_hsv(cnts,FP,imageA,imageB)
    Diff_image_animals=list_hsv(cnts,liste_Diff_animals,imageA,imageB)
    Diff_image_total=list_hsv(cnts,range(len(cnts)),imageA,imageB)
    
    #batch_to_select=batchImages_FP+batchImages_match_animals
    #batchimages_to_select
    #Peut être ici pour les pixels
...
def predict_8classes(name_test,name_ref,folder,CNNmodel,numb_classes,mask=True,coverage_threshold=0.99,contrast=-5,blockSize=53,blurFact=15,
    ...
    #if mask==True:
    #    
        #imageA = np.multiply(imageA, mask) 
        #imageB=np.multiply(imageB, mask) 
    #Ouverture des fichiers annotés 
    imagettes=pd.read_csv("/mnt/VegaSlowDataDisk/c3po/Images_aquises/imagettes.csv")
    nom_classe,imagettes_target=open_imagettes_file(imagettes,folder,name_test)
    #On effectue une différentiation des images et on récupères le filtre des images sous forme de batch
    ...
    #C'est peut être ici qu'il faut mettre le masque du genre. imageB=mask(imaageB)
    #batchImages_stack_reshape,table_non_filtre=batched_cnts(cnts,imageB)
    if mask==True:
        print("masque activée")
        mask = load(mask_path)
        ImageMaksed=np.multiply(imageB, mask) 
        imageB=ImageMaksed.astype(int)
    #plt.imshow(imageB)
    batchImages_mask,table_non_filtre=batched_cnts(cnts,imageB)
    ...
    #On classe les imagettes générées en fonction de l'espèce avec laquelle ses coordonnées correspondent sur l'image
    liste_batch_images=range(len(batchImages_mask))
    (liste_Diff_animals,dict_anotation_index_to_classe,liste_DIFF_birds_defined,liste_DIFF_birds_undefined,birds_defined_match,liste_DIFF_corbeau,
     liste_DIFF_faisan,liste_DIFF_pigeon,liste_DIFF_other_animals)=class_imagettes_bis(generate_square,coverage_threshold, nom_classe,imagettes_target,dic_labels_to_num) 
    ...
    #On effectue les predictions et on les classes les index selon l'espèce
    estimates = CNNmodel.predict(batchImages_mask)
    #estimates = CNNmodel.predict(batchImages_mask)
    liste_prediction=list(estimates.argmax(axis=1))
    liste_prediction_animals_match=[liste_prediction[i] for i in liste_Diff_animals]
    index_others,index_birds,index_other_animals,index_chevreuil,index_corbeau,index_lapin,index_faisan,index_pigeon=class_predictions_mclasses(liste_prediction,numb_classes)
    ...
    """
    draw_rectangle(FP_square,"Blue",imageRectangle)
    image_path="/mnt/VegaSlowDataDisk/c3po_interface_mark/bin/birds_wmsss.jpg"
    cv2.imwrite(image_path,imageRectangle)
    """
    
    #On ne garde que les prédictions au-dessus de 0.9
    if thresh_active==True:
        birds_pr_birds_thresh=[i for i in birds_pr_bird_estimates if i > thresh]
        FP_birds_thresh=[i for i in FP_estimates if i > thresh]
        print("birds_pr_birds_thresh",birds_pr_birds_thresh)
        ...
        FP_square=generate_square.loc[FP]
        draw_rectangle(FP_square,"Blue",imageRectangle)
        image_path="/mnt/VegaSlowDataDisk/c3po_interface_mark/bin/image_squared/birds.jpg"
        cv2.imwrite(image_path,imageRectangle)            
        """
...
def extract_dist_by_type_shorter(name_test,name_ref,folder,CNNmodel,numb_classes
    ...
    Diff_image_animals=diff_im_slow_bis(cnts,liste_Diff_animals,imageA,imageB)
