...
c3poFolder="/mnt/mnt/BigFast/VegaFastExtension/Rpackages/c3po_all/c3po_interface_mark/"
...
#mnt/BigFast/VegaFastExtension/Rpackages/c3po_all
#
...
def diff_images(imageA,imageB,contrast=-5,blockSize=51,blurFact = 25,method="light"):
...
def filtre_light(imageA,imageB,contrast=-5,blockSize=51,blurFact = 25,method="light"):
...
def filtre_khalid(imageA,imageB):
...
#Possible de le réecirire mieux
...
def extract_birds(name_test,name_ref,folder,diff_mod="HSV",coverage_threshold=0.99):
    ...
    liste_DIFF_corbeau,liste_DIFF_faisan,liste_DIFF_pigeon,liste_DIFF_lapin,liste_DIFF_chevreuil)=class_imagettes_sans_dboucle(generate_square,coverage_threshold, nom_classe,imagettes_target,dic_labels_to_num,precise=True)
...
def mask_function(folder,cnts,imageB,mask):
    ...
    masks_path='/mnt/VegaSlowDataDisk/BigFast/VegaFastExtension/Rpackages/c3po_all/find_dominant_colors/how_to/masque/'
...
"""
miss_well_class(estimates,liste_Diff_birds,liste_DIFF_not_matche,
...
def miss_well_class(estimates,liste_Diff_birds,liste_DIFF_not_matche,
    ...
    #On peut éventuellement inverse birds et other
    ...
    #index_birds,index_others,index_other_animals,index_chevreuil,index_corbeau,index_lapin,index_faisan,index_pigeon= class_predictions_dictionnaire(liste_prediction,class_num)
    index_animals=index_birds+index_other_animals
    
    #Ce que je veux, c'est avoir l'index et non pas le num
    # Ce que j'ai dans list diff
...
def colors_imagettes(given_name,name_test,name_ref,folder,CNNmodel,numb_classes,to_Select="FP",contrast=-5,blockSize=17,blurFact=15,
            filtre_choice="No_filtre", thresh=0.9, thresh_active=True):
    
    
    (imageA,imageB,cnts,batchImages_stack_reshape,generate_square,
    TP,FP,TP_thresh_index,FP_thresh_index)= base_4C_poly(name_test,name_ref,folder,CNNmodel,numb_classes,index=True,mask=True,blockSize=17,thresh=0.9)
    
    
    #Draw Rectangles
    imageRectangle=imageB.copy()
    if len(TP_thresh_index)>0:
        birds_square=generate_square.loc[TP_thresh_index]
        imageRectangle=draw_rectangle(birds_square,"Green",imageRectangle)
        
    
    if len(FP_thresh_index)>0:
        FP_square=generate_square.loc[FP_thresh_index]
        imageRectangle=draw_rectangle(FP_square,"Blue",imageRectangle)
        
    image_path="/mnt/VegaSlowDataDisk/c3po_interface_mark/Materiels/pictures/image_demonstration/image_squared/"+given_name
...
def extract_size(name_test,name_ref,folder,CNNmodel,diff_mod="GBR",
    ...
    batchImages_stack_reshape,table_non_filtre=batched_cnts(cnts,imageB) 
    #J'ai changé le filtre line si pb version avant
    ...
    (liste_Diff_animals,dict_anotation_index_to_classe,liste_DIFF_birds_defined,liste_DIFF_birds_undefined,birds_defined_match,liste_DIFF_corbeau,
     liste_DIFF_faisan,liste_DIFF_pigeon,liste_DIFF_other_animals)=class_imagettes_sans_dboucle(generate_square,coverage_threshold, nom_classe,imagettes_target,dic_labels_to_num) 
...
def base_4C_poly(name_test,name_ref,folder,CNNmodel,diff_mod="HSV",
                                 chanels=3,numb_classes=6,mask=False,coverage_threshold=0.99,contrast=-5,blockSize=53,blurFact=15,filtre_choice="No_filtre", thresh=0.99, thresh_active=True,index=False,down_thresh=100):
    ...
    if batchImages_stack_reshape!=[]:
        ...
        TP_birds,FP,TP_estimates,FP_estimates= miss_well_class(estimates,liste_Diff_birds,liste_DIFF_not_matche,
               ...
               thresh_active,thresh,numb_classes=6,focus="bird_large",index=index)
    else:
...
def base_Khalid(name_test,name_ref,folder,
                                 numb_classes=2,mask=True,coverage_threshold=0.99,contrast=-5,blockSize=53,blurFact=15,filtre_choice="No_filtre", thresh=0.99, thresh_active=True,index=False,down_thresh=25):
    ...
    if batchImages_stack_reshape!=[]:
       ...
       """
        ...
        print("Tp_birds",TP_birds)
        """
        imageRectangle=imageB.copy()
        imageSSIM=draw_rectangle(generate_square,"White",imageRectangle)
      
        rec_im="/mnt/VegaSlowDataDisk/c3po_interface_mark/Materiels/pictures/image_demonstration/test/"
        cv2.imwrite(rec_im+"ssim_3birds.JPG",imageSSIM)
        """ 
...
#Il y a un probleme ici le class_num et numb_classes=6 ne servent à rien,en dessous focus=animals
#Je pense que cette fonction n'a jamais servi
def base_option(name_test,name_ref,folder,CNNmodel,
                                 method="ssim",numb_classes=6,mask=True,coverage_threshold=0.5,contrast=-8,blockSize=17,blurFact=17,filtre_choice="No_filtre", thresh=0.99, thresh_active=True,index=False,down_thresh=25):
    ...
    cnts=diff_images(imageA,imageB,method=method)
    if cnts!=[]:
        #batchImages_stack_reshape,table_non_filtre=batched_cnts(cnts,imageB,224) 
        batchImages_stack_reshape,table_non_filtre=batched_cnts(cnts,imageB,28) 
        generate_square,batchImages_stack_reshape=filtre_line_bis(table_non_filtre,filtre_choice,batchImages_stack_reshape,down_thresh)
        batchImages_stack_reshape=np.array(batchImages_stack_reshape)
    else:
        batchImages_stack_reshape=np.zeros(shape=(0,0))
    ...
    if  batchImages_stack_reshape.size :
        ...
        nb_imagettes=len(batchImages_stack_reshape)
        batchImages_stack_reshape,generate_square=pre_select(batchImages_stack_reshape,generate_square,cnts,50)
        """
        features = model.predict(np.array(batchImages_stack_reshape), batch_size=4) # why 5 if only 4 proc on the pi...
        features = features.reshape((features.shape[0], 7 * 7 * 512))
        ...
        TP_birds,FP,TP_estimates,FP_estimates= miss_well_class(estimates,liste_Diff_birds,liste_DIFF_not_matche,
                        liste_DIFF_corbeau,liste_DIFF_faisan,liste_DIFF_pigeon,liste_Diff_animals,thresh_active,thresh,numb_classes=numb_classes,focus="animals",index=True)
        print(TP_birds)
        ...
        TP_birds,FP,TP_estimates,FP_estimates=[[] for i in range(4)]
        birds_defined_match,nb_imagettes=(0,0)
    return imageA,imageB,cnts,batchImages_stack_reshape,generate_square,TP_birds,FP,TP_estimates,FP_estimates,birds_defined_match,nb_imagettes





def pre_select(batchImages_stack_reshape,generate_square,cnts,maxAnalDL=12):
...
def base_4C_poly_bis(name_test,name_ref,folder,CNNmodel,maxAnalDL,diff_mod="HSV",method="light",
                                 chanels=3,numb_classes=6,mask=False,coverage_threshold=0.99,contrast=-5,blockSize=53,blurFact=15,filtre_choice="No_filtre", thresh=0.99, thresh_active=True,index=False,down_thresh=25):
    #Faire un seuil à 29 de surface
    #Parameters
    imageSize=28
    #Dictionnary to convert string labels to num labels
    ...
    #Organize generated imagettes and apply filters
    #cnts=filtre_light(imageA,imageB,blockSize=blockSize,contrast=contrast,blurFact=blurFact)
    #cnts=diff_images(imageA,imageB,contrast=contrast,blockSize=blockSize,blurFact = blurFact,method=method)
    cnts=diff_images(imageA,imageB,method=method)
    ...
    if  cnts :
        ...
        if len(generate_square)!=0:
            batchImages_stack_reshape,generate_square=pre_select(batchImages_stack_reshape,generate_square,cnts,maxAnalDL=maxAnalDL)
