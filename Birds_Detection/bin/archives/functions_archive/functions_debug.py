...
# -*- coding: utf-8 -*-
"""
Created on Thu Apr  9 10:42:31 2020
...
def area_intersection(x_min_gen,x_max_gen,y_min_gen,y_max_gen,  x_min_anote,x_max_anote,y_min_anote,y_max_anote   ):
    ...
    min_xmax=min(x_max_gen,x_min_anote)
    max_xmin=max(x_min_gen,x_max_anote)
    min_ymax=max(y_max_gen,y_max_anote) 
...
def birds_square_light(path_images,name_test,name_ref,coverage_threshold,limit_area_square,contrast,neurone_features,blockSize,blurFact,folder,filtre_choice="No_filtre",coef_filtre=coef_filtre,height=2448,width=3264):
    ...
    xmin_gen=imagettes["xmin"]
    xmax_gen=imagettes["xmax"]
    ymin_gen=imagettes["ymin"]
    ymax_gen=imagettes["ymax"]


    #Il faudra bien sûr faire une boucle pour ça
    ...
    y_max_anote=annontation_reduit["xmax"].iloc[i]
    ...
    if proportion_maximum>limit_area_square:
